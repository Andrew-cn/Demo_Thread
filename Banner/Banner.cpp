...
#define OUTPUT_TEXT_COLOR BACKGROUND_BLUE | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE
...
///////////////////////////////////////////////////////////////////////////////
// Structure passed to thread on startup
//
typedef struct {
...
int main() {
	...
	hRunObject = CreateEvent( NULL, TRUE, 0, NULL );
...
void ShutDownThreads( void ) {
	if (nThreads > 0) {
		...
		SetEvent(hRunObject);
		WaitForMultipleObjects(nThreads, ThreadHandles, TRUE, INFINITE) != WAIT_FAILED;
		while (--nThreads) CloseHandle( ThreadHandles[nThreads] );
	}
}
...
void MainLoop( void ) {
	...
	for (;;) {
		...
		ReadFile( hConsoleIn, buf, INPUT_BUF_SIZE-1, &bytesRead, NULL );
		// ReadFile is binary, not line oriented,
		// so terminate the string.
		buf[bytesRead] = '\0';
		FlushConsoleInputBuffer( hConsoleIn );
		if (StripCr( buf ) == 0) break;
		...
		if (nThreads < MAX_THREADS) {
			...
			data_block = (DataBlock *) HeapAlloc( GetProcessHeap(), HEAP_ZERO_MEMORY,
				sizeof(DataBlock) );
			...
			ThreadHandles[nThreads++] = CreateThread(NULL, 0, BannerProc, data_block,
				0, &thread_id);
		}
	}
}
...
DWORD WINAPI BannerProc( LPVOID pParam ) {
	...
	for (i=len; i<BANNER_SIZE; ++i) OutputBuf[i] = ' ';
	...
	do {
		...
		if (ScrollPosition == len) ScrollPosition = 0;

		WriteConsoleOutput( hConsoleOut, CharBuf, Size, TopLeft, &rect );
