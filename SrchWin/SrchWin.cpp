...
// so that is can use the single-threaded
// C libraries.
//
...
int main(int argc, char * argv[]) {
	...
	// to the first thread
	lpFindData = (WIN32_FIND_DATA *) HeapAlloc( GetProcessHeap(), HEAP_ZERO_MEMORY,
	...
	do {
		...
		hThread = CreateThread(NULL, 0, SearchProc, lpFindData, 0, &dummy);
...
DWORD WINAPI SearchProc( void * arg ) {
	...
	hFile = CreateFile(lpFindData->cFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
		FILE_FLAG_SEQUENTIAL_SCAN, NULL);

	if (!hFile) return EXIT_FAILURE; // Silently ignore problem files
	...
	while (GetLine(hFile, buf, sizeof(buf))) {
		...
		if (strstr(buf, szSearchFor)) {
			...
			WriteFile( hConsoleOut, lpFindData->cFileName, strlen(lpFindData->cFileName),
				&dummy, FALSE );
...
// automaticallly. Calling ReadFile() one character
