...
int main() {
	...
	if(hThrd1) printf("Thread 1 launched\n");

	hThrd2 = CreateThread(NULL, 0, ThreadFunc, (LPVOID)2, 0, &threadId);
	if(hThrd2) printf("Thread 2 launched\n");
	...
	for(;;) {
		...
		if(exitCode1 == STILL_ACTIVE) puts("Thread 1 is still running!");
		if(exitCode2 == STILL_ACTIVE) puts("Thread 2 is still running!");
		if(exitCode1 != STILL_ACTIVE && exitCode2 != STILL_ACTIVE) break;
	}
